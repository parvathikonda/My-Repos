...
class Auth_Multisite_Model {
    ...
    public function __construct($htpasswdPath, $serialsPath, $secretPath, $loginUrl) {
        ...
        $this->serialsPath  = dirname($secretPath) . "/auth.serials";
    ...
    private function serialOfUser($username) {
        if (!file_exists($this->serialsPath))
            return '0';
        foreach(file($this->serialsPath) AS $line) {
            if (strlen($line) > 0 && $line[0] != '#') {
                list($un, $serial) = explode(':', $line, 2);
                if ($un == $username) {
                    return rtrim($serial);
                }
            }
